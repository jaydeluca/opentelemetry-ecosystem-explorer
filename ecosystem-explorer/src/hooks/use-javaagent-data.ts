...
export function useInstrumentation(id: string, version: string): DataState<InstrumentationData> {
  const [state, setState] = useState<DataState<InstrumentationData>>({
    data: null,
    loading: true,
    error: null,
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        const data = await javaagentData.loadInstrumentation(id, version);
        if (!cancelled) {
          setState({ data, loading: false, error: null });
        }
      } catch (error) {
