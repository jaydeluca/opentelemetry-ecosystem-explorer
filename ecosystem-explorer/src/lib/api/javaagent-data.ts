...
let idbEnabled = idbCache.isIDBAvailable();

export async function initDataLoader(): Promise<void> {
  if (idbEnabled) {
    try {
      await idbCache.initDB();
      console.log("Data loader initialized with IndexedDB");
    } catch (error) {
      console.warn("IndexedDB unavailable, using memory-only cache:", error);
      idbEnabled = false;
    }
  }
}
...
async function fetchWithCache<T>(
  cacheKey: string,
  url: string,
  storeType: "metadata" | "instrumentations" = "instrumentations"
): Promise<T> {
  ...
  const request = (async () => {
    try {
      if (idbEnabled) {
        const cachedData = await idbCache.getCached<T>(cacheKey);
        if (cachedData !== null) {
      ...
      if (idbEnabled) {
        await idbCache.setCached(cacheKey, data, storeType);
      }
